...
var statusMessages = helpers.statusMessages;
var responseTypes = helpers.responseTypes;
var notAllowedHeaders = helpers.notAllowedHeaders;
var unsupportedMethods = helpers.unsupportedMethods;
var supportedMethods = helpers.supportedMethods;

var parseUri = helpers.parseUri;
var makeXMLResponse = helpers.makeXMLResponse;
var callOldSchoolCallbackOnEvent = helpers.callOldSchoolCallbackOnEvent;
var createProgressEvent = helpers.createProgressEvent;

var XMLHttpRequestUpload = helpers.XMLHttpRequestUpload;
var XMLHttpRequestEventTarget = helpers.XMLHttpRequestEventTarget;
...
function XMLHttpRequest() {
    ...
    callOldSchoolCallbackOnEvent.call(this, 'readystatechange');
}
...
extend(XMLHttpRequest.prototype, {
    ...
    setRequestHeader: function setRequestHeader(header, value) {
        ...
        if (notAllowedHeaders.indexOf(header.toLowerCase()) !== -1) {
            throw Error('Refused to set unsafe header "' + header + '"');
        }

        header = header.toLowerCase();

        if (header.toLowerCase().substr(0, 6) === 'proxy-' || header.toLowerCase().substr(0, 4) === 'sec-') {
            // silent fail
            return;
        }
    ...
    send: function send(data) {
        ...
        server.receive({
            ...
            onresponse: (function (status, response) {
                ...
                if (mimetype === null || mimetype === 'text/xml' || mimetype === 'application/xml' || mimetype && mimetype.substring(mimetype.length - 4) === '+xml') {
